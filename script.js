...
// --- SÉLECTEURS ÉLÉMENTS ---
    ...
    const navLinks = document.querySelectorAll('.nav-link'); // <-- C'était manquant !
...
// --- SCROLLSPY : DETECTION DES SECTIONS ET BULLETS DYNAMIQUES ---
    const targetIds = Array.from(navLinks).map(link => link.getAttribute('href').split('#')[1]);
    const sections = document.querySelectorAll(targetIds.map(id => `[id="${id}"]`).join(','));
...
// --- DETECTION ACTIVE DES CARTES SUR MOBILE (EFFET HOVER AU SCROLL) ---
    if (isMobile()) {
            ...
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting) {
                    card.classList.add(
                        ...
                        'via-[#a370f7]/40',
                        'to-cyan-400/40',
                        'shadow-[0_0_25px_rgba(132,73,217,0.3)]'
                    );
...
// --- EFFET DE COULEUR AU SCROLL SUR LES IMAGES (MOBILE UNIQUEMENT) ---
    const setupMobileImagesScroll = () => {
        ...
        const imgObserverCallback = (entries) => {
            entries.forEach(entry => {
                ...
                const isUnsplash = img.getAttribute('src').includes('unsplash.com');
                if (entry.isIntersecting) {
                    if (isUnsplash) {
    ...
    const setupEmailProtection = () => {
        document.querySelectorAll('.protected-mail').forEach(link => {
            ...
            try {
                ...
                if (textContainer) {
                    textContainer.textContent = fullEmail;
                }
            } catch (e) {
...
// --- GESTION DU FILTRE REALISATIONS ---
function filterProjects(category) {
    ...
    buttons.forEach(btn => {
        ...
        if (span) {
            span.className = "text-span bg-gradient-to-r from-[#8449d9] to-cyan-400 bg-clip-text text-transparent group-hover/filter:text-white transition-colors duration-300 flex items-center gap-2";
        }
        const svg = btn.querySelector('svg');
        if (svg) {
            svg.classList.remove('text-white');
            svg.classList.add('text-[#8449d9]');
        }
    });
    ...
    if (activeBtn) {
        ...
        if (activeSpan) {
            activeSpan.className = "text-span text-white flex items-center gap-2";
        }
        const activeSvg = activeBtn.querySelector('svg');
        if (activeSvg) {
            activeSvg.classList.remove('text-[#8449d9]');
...
// --- GESTION DES BANNIÈRES LINKEDIN (MOBILE) ---
...
// --- NAVIGATION CAROUSEL BANNIÈRES (DESKTOP) ---
...
// --- GESTION DU SWIPE CARTE CARROU INSTA ---
    function updateCarouselCounter(element) {
        const counter = document.getElementById('carousel-counter');
        if (!counter) return;
        const slideWidth = element.offsetWidth;
        const currentSlide = Math.round(element.scrollLeft / slideWidth) + 1;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    openWebButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            if (webModal && webModalImg && webModalCta) {
                if (window.innerWidth >= 768 && imgDesktop) {
                    webModalImg.setAttribute('src', imgDesktop);
                } else {
                    webModalImg.setAttribute('src', imgMobile);
                }
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && webModal && !webModal.classList.contains('hidden')) {
            closeWebLightbox();
        }
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    openLinkedinButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            if (linkedinModal && linkedinModalImg) {
                if (window.innerWidth >= 768 && imgDesktop) {
                    linkedinModalImg.setAttribute('src', imgDesktop);
                } else {
                    linkedinModalImg.setAttribute('src', imgMobile);
                }
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && linkedinModal && !linkedinModal.classList.contains('hidden')) {
            closeLinkedinModal();
        }
    });
});